...
async function _shiping() {
    ...
    if (!_dir) {
        console.warn(`${pkg.name} >> Extension asset directory not found`);
        return;
    }
...
function _getConfigDir() {
    const physic = path.resolve(__dirname, '..', 'assets', '_$plugins');
    const db = `db://${pkg.name}/_$plugins`;
...
`;
    ...
    const jsPath = path.join(_dir.physic, `${PLUGIN_NAME}.js`);
    const dtsPath = path.join(_dir.physic, `${PLUGIN_NAME}.d.ts`);
    const jsUrl = `${_dir.db}/${PLUGIN_NAME}.js`;
    const dtsUrl = `${_dir.db}/${PLUGIN_NAME}.d.ts`;
    ...
    try {
        ...
        const __objMeta = await Editor.Message.request('asset-db', 'query-asset-meta', jsUrl) as IAssetMeta;
        ...
        console.log(`[${pkg.name}] Successfully generated bundle_list plugin files inside extension at: ${jsUrl}`);
    } catch (e) {
